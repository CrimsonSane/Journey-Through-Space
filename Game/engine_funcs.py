...
def create_zone_text():
    objects.Moving_text("ZONE:"+str(engine_vars.zone_id), [engine_vars.WIN_WIDTH/2,-10],
                        [0,engine_vars.scroll_spd], engine_vars.mving_txt_group)
...
def create_astroids(amount, group):
    for i in range(amount):
        objects.Astroid(pos=[random.randint(0,engine_vars.WIN_WIDTH),random.randint(-engine_vars.WIN_HEIGHT,0)],
...
def create_scrap(amount, group):
    for i in range(amount):
        objects.Scrap(pos=[random.randint(0,engine_vars.WIN_WIDTH),random.randint(-engine_vars.WIN_HEIGHT,0)],
...
def play_music(music):
    if not pygame.mixer.music.get_busy() or engine_vars.current_music != music:
...
def play_random_music():
    while 1:
        rand_song = random.randint(1,len(engine_vars.MUSIC_TRACKS) - 1)
        if engine_vars.current_music != engine_vars.MUSIC_TRACKS[rand_song]:
            return play_music(engine_vars.MUSIC_TRACKS[rand_song])
...
def create_items():
    ...
    #print(round(engine_vars.current_tick/SPAWN_INTERVAL,2), round(engine_vars.current_tick/SPAWN_INTERVAL,2).is_integer())
    if round(engine_vars.current_tick/SPAWN_INTERVAL,2).is_integer():
        ...
        if chance == chance_target:
            objects.Item(ITEM_NAMES[ran_item_index], [random.randint(0,engine_vars.WIN_WIDTH),random.randint(-engine_vars.WIN_HEIGHT,0)],
                         [0,6], engine_vars.item_group)
